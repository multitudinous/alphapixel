...
void ConfigFile::extractContents(const std::string &line) 
{
	...
	{
		// Duplicate keys are allowed (GROUP and END_GROUP are re-used for each MTL section).
		// The newly read value replaces old value for the key.
		// Modified by Gary Huber.
		contents[key] = value;
...
void ConfigFile::parseLine(const std::string &line, size_t const lineNo)
{
	if (line.find('=') == line.npos)
	{
		// End of file hopefully, and not an error so we will return gracefully without parsing the line.
		// Modified by Gary Huber.
		return;
